        ...
        while True:
            ...
            while not is_terminal:
                ...
                if is_terminal:
                    ...
                    average_action_probs = np.mean(action_probs_list, axis=0)
                    self.action_probabilities.append(average_action_probs)
...
if __name__ == "__main__":
    ...
    # Plot Average Action Probabilities over Episodes
    action_probs = np.array(agent.action_probabilities)
    plt.figure(figsize=(12, 6))
    for i in range(agent.num_actions):
        plt.plot(action_probs[:, i], label=f'Action {i} Probability')
    plt.xlabel('Episode')
    plt.ylabel('Average Action Probability')
    plt.title('Average Action Probabilities over Episodes')
    plt.legend()
    plt.show()